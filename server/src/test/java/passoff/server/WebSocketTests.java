...
class WebSocketTests {
    ...
    void makeMoveBadAuthtoken() {
        ...
        makeMove(
                new WebsocketUser(white.username(), "badAuth"),
                gameID,
                move,
                false,
                false,
    ...
    private
    WebsocketUser registerUser(String name, String password, String email) {
    ...
    private
    int createGame(WebsocketUser user, String name) {
    ...
    private
    void joinGame(int gameID, WebsocketUser user, ChessGame.TeamColor color) {
    ...
    private
    void assertHttpOk(TestResult result, String context) {
    ...
    private
    void connectToGame(WebsocketUser sender, int gameID, boolean expectSuccess,
    ...
    private
    void makeMove(WebsocketUser sender, int gameID, ChessMove move, boolean expectSuccess,
    ...
    private
    void resign(WebsocketUser sender, int gameID, boolean expectSuccess,
                Set<WebsocketUser> inGame, Set<WebsocketUser> otherClients) {
    ...
    private
    void leave(WebsocketUser sender, int gameID, Set<WebsocketUser> inGame, Set<WebsocketUser> otherClients) {
    ...
    private
    Map<String, Integer> expectedMessages(WebsocketUser sender,
                                          int senderExpected,
        ...
        expectedMessages.putAll(inGame.stream()
                                      .collect(Collectors.toMap(WebsocketUser::username, s -> inGameExpected)));
        expectedMessages.putAll(otherClients.stream().collect(Collectors.toMap(WebsocketUser::username, s -> 0)));
        return expectedMessages;
    }

    private
    void assertCommandMessages(Map<String, List<TestMessage>> messages, boolean expectSuccess,
    ...
    private
    void assertMessages(String username, ServerMessage.ServerMessageType[] expectedTypes, List<TestMessage> messages) {
    ...
    private
    void assertLoadGame(String username, TestMessage message) {
        ...
        Assertions.assertNotNull(
                message.getGame(),
                "%s's LOAD_GAME message did not contain a game (Make sure it's specifically called 'game')".formatted(
                        username)
        );
    ...
    private
    void assertNotification(String username, TestMessage message) {
        ...
        Assertions.assertNull(
                message.getErrorMessage(),
                "%s's NOTIFICATION message contained an error message: %s".formatted(
                        username,
                        message.getErrorMessage()
                )
        );
    ...
    private
    void assertError(String username, TestMessage message) {
    ...
    private
    ServerMessage.ServerMessageType[] types(ServerMessage.ServerMessageType... types) {
        return types;
    }

    private static
    record WebsocketUser(String username, String authToken) {}
}